...
object FirebaseModule {
    ...
    fun providesFirebaseAppCheck(): FirebaseAppCheck {
        ...
        if (com.silversentry.sentry.BuildConfig.BUILD_TYPE != "release") {
    ...
    fun providesFirebaseAuth(): FirebaseAuth = FirebaseAuth.getInstance().apply {
        if (com.silversentry.sentry.BuildConfig.USE_LOCAL_BACKEND) {
            useEmulator(com.silversentry.sentry.BuildConfig.LOCAL_BACKEND_HOST, 9099)
        }
    }
    ...
    fun providesFirebaseFirestore(): FirebaseFirestore = FirebaseFirestore.getInstance().apply {
        if (com.silversentry.sentry.BuildConfig.USE_LOCAL_BACKEND) {
            useEmulator(com.silversentry.sentry.BuildConfig.LOCAL_BACKEND_HOST, 8080)
        }
    }
    ...
    fun providesFirebaseStorage(): FirebaseStorage = FirebaseStorage.getInstance().apply {
        if (com.silversentry.sentry.BuildConfig.USE_LOCAL_BACKEND) {
            useEmulator(com.silversentry.sentry.BuildConfig.LOCAL_BACKEND_HOST, 9199)
        }
    }
    ...
    fun providesFirebaseFunctions(): FirebaseFunctions = FirebaseFunctions.getInstance().apply {
        if (com.silversentry.sentry.BuildConfig.USE_LOCAL_BACKEND) {
            useEmulator(com.silversentry.sentry.BuildConfig.LOCAL_BACKEND_HOST, 5001)
        }
    }
}
